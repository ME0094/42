...
int main(void) { char d[16]; printf("%s", ft_strcpy(d, "Piscine")); return (0); }
#elif defined(TEST_STRLEN)
int ft_strlen(char *);
int main(void) { printf("%d,%d", ft_strlen("Piscine"), ft_strlen("")); return (0); }
#elif defined(TEST_PUTSTR)
void ft_putstr(char *);
int main(void) { ft_putstr("42 Piscine"); return (0); }
#elif defined(TEST_SWAP)
void ft_swap(int *, int *);
int main(void) { int a = -4; int b = 42; ft_swap(&a, &b); printf("%d,%d", a, b); return (0); }
...
char *ft_strdup(char *);
int main(void) { char *s = ft_strdup("copy me"); if (!s) return (1); printf("%s", s); free(s); return (0); }
#elif defined(TEST_REVERSE_BITS)
unsigned char reverse_bits(unsigned char);
int main(void) { printf("%u", (unsigned int)reverse_bits(0x12)); return (0); }
#elif defined(TEST_SWAP_BITS)
unsigned char swap_bits(unsigned char);
int main(void) { printf("%u", (unsigned int)swap_bits(0x41)); return (0); }
...
int main(void) { print_bits(42); return (0); }
...
int main(void) { int i; int *a = ft_rrange(1, -2); if (!a) return (1); i = 0; while (i < 4) { if (i) printf(","); printf("%d", a[i++]); } free(a); return (0); }
...
int main(void) { t_list *a = malloc(sizeof(*a)); t_list *b = malloc(sizeof(*b)); t_list *c = malloc(sizeof(*c)); t_list *p; if (!a || !b || !c) return (1); a->data="x"; a->next=b; b->data="keep"; b->next=c; c->data="x"; c->next=0; ft_list_remove_if(&a, "x", same); p=a; while(p){printf("%s",(char *)p->data);p=p->next;} free(a); return (0); }
...
int main(void) { t_list c={2,0}; t_list b={1,&c}; t_list a={3,&b}; t_list *p=sort_list(&a,ascending); while(p){printf("%d",p->data);p=p->next;} return (0); }
...
int main(void) { char a[]="1110";char b[]="1000";char c[]="1110";char *t[]={a,b,c};int i;flood_fill(t,(t_point){4,3},(t_point){0,0});i=0;while(i<3){printf("%s",t[i++]);if(i<3)printf("/");}return(0); }
#elif defined(TEST_LIST_FOREACH)
typedef struct s_list { struct s_list *next; void *data; } t_list;
void ft_list_foreach(t_list *, void (*)(void *));
static void inc(void *p) { (*(int *)p)++; }
int main(void) { int x=1,y=4;t_list b={0,&y};t_list a={&b,&x};ft_list_foreach(&a,inc);printf("%d,%d",x,y);return(0); }
#elif defined(TEST_SPLIT)
char **ft_split(char *);
int main(void) { char **a=ft_split("  one\ttwo three ");int i=0;if(!a)return(1);while(a[i]){if(i)printf(",");printf("%s",a[i]);free(a[i++]);}free(a);return(0); }
#elif defined(TEST_SORT_INT)
void sort_int_tab(int *, unsigned int);
int main(void) { int t[]={3,-1,3,0};int i=0;sort_int_tab(t,4);while(i<4){if(i)printf(",");printf("%d",t[i++]);}return(0); }
#elif defined(TEST_PRINT_MEMORY)
#include <stddef.h>
void print_memory(const void *, size_t);
int main(void) { const unsigned char s[]="Hello";print_memory(s,5);return(0); }
